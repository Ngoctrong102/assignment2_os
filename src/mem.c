...
addr_t alloc_mem(uint32_t size, struct pcb_t *proc)
{
	...
	if (mem_avail)
	{
		...
		for (int i = 0; i < num_pages; i++)
		{
			...
			if (page_table == NULL)
			{
				if (proc->seg_table->size < (1 << PAGE_LEN))
				{
...
int free_mem(addr_t address, struct pcb_t *proc)
{
	...
	// printf("hghghghgghghghghghghghghghghghghghghghghghghgh1");
	if (translate(address, p_addr, proc) == 0)
		return 0;
	// printf("qqwqwqwqwqwqwqwqwqwqwqwqwqwqwqwqwqwqwqwqwqw2");
	while (next != -1)
	{
		next = _mem_stat[(*p_addr) >> OFFSET_LEN].next;
		// printf("hahhahaahahahahaha %d",p_addr >> OFFSET_LEN);
